...
function BladeLib:CreateWindow(config)
	...
	function Window:AddTab(tabConfig)
		...
		ScrollContainer.BackgroundTransparency = 0.95
		ScrollContainer.BorderSizePixel = 0
		ScrollContainer.ScrollBarThickness = 3
		ScrollContainer.ScrollBarImageColor3 = WHITE
		ScrollContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
		ScrollContainer.ZIndex = 7
		...
		local function activateTab()
			if activeTab then
				...
				TweenService:Create(prevWrap, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {GroupTransparency = 1}):Play()
				task.delay(0.16, function() prevWrap.Visible = false end)
			end
